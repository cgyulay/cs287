...
For the naive Bayes model, we first compiled a prior probability matrix $\boldY_{\scriptscriptstyle 1\times c}$ to model $P(y)$. We then built a class-word probability matrix $\boldM_{\scriptscriptstyle c\times \mcV}$ that indicates the probability that a word is associated with a given class. $\boldM$ models $P(x|y)$.
